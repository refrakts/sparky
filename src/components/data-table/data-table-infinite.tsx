...
export function DataTableInfinite<TData, TValue>({
    ...
    const [columnOrder, setColumnOrder] = useLocalStorage<string[]>(getColumnOrderKey(tableId), []);
        ...
        >
            ...
            >
                ...
                >
                    <div
                        ref={topBarRef}
                        className={cn('bg-background flex flex-col gap-4 p-2', 'sticky top-0 z-10 pb-4')}
                    >
                        {commandSlot}
                        {/* TBD: better flexibility with compound components? */}
                        <DataTableToolbar renderActions={toolbarActions ? () => toolbarActions : undefined} />
                        {chartSlot}
                    </div>
                    <div className="z-0">
                        <Table
        ...
        >
            {row.getVisibleCells().map((cell) => (
                <TableCell
                    ...
                    className={cn('border-border truncate border-b', cell.column.columnDef.meta?.cellClassName)}
                >

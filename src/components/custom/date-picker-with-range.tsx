...
export function DatePickerWithRange({ className, date, setDate, presets = defaultPresets }: DatePickerWithRangeProps) {
...
function DatePresetsSelect({
    ...
    const [value, setValue] = React.useState<string | undefined>(findPreset(selected?.from, selected?.to));

...
function AiChat({ status = 'ready', children, className }: AiChatProps) {
    ...
    return (
        <AiChatContext.Provider value={contextValue}>
            <div data-slot="ai-chat" className={cn('flex h-full flex-col bg-background', className)}>
                {children}
            </div>
...
function AiChatFooter({ children, className }: AiChatFooterProps) {
    return (
        <div data-slot="ai-chat-footer" className={cn('bg-background px-4 py-3', className)}>
            {children}
        </div>
    );
}
...
export { AiChat, AiChatBody, AiChatFooter, AiChatHeader, useChatContext };

 ...
 * ```
 */
export function useFilterState<T extends Record<string, unknown>, R = T>(selector?: (state: T) => R): R {
    const context = useStoreContext();

    if (!context) {
        throw new Error('useFilterState must be used within a DataTableStoreProvider');
    }

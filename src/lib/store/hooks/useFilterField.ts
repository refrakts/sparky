 ...
 * ```
 */
export function useFilterField<T extends Record<string, unknown>, K extends keyof T>(key: K): FilterFieldResult<T[K]> {
    const context = useStoreContext();

    if (!context) {
        throw new Error('useFilterField must be used within a DataTableStoreProvider');
    }

    const { adapter } = context;

    const subscribe = useCallback((onStoreChange: () => void) => adapter.subscribe(onStoreChange), [adapter]);

import type { FieldBuilder, InferSchemaType, SchemaDefinition } from '../schema/types';
...
export function createTextParser<T extends SchemaDefinition>(
    ...
    const { aliases = {}, fieldDelimiter = ' ', keyValueDelimiter = ':' } = options;
    ...
    return {
        parse(input: string): Partial<InferSchemaType<T>> {
            ...
            const parts = input.trim().split(new RegExp(`\\s*${escapeRegex(fieldDelimiter)}\\s*`));
            ...
            for (const part of parts) {
                ...
                const fieldBuilder = schema[fieldKey] as FieldBuilder<unknown> | undefined;
        ...
        replaceWordAtCaret(input: string, caretPosition: number, replacement: string) {
        ...
        getSuggestions(input: string, caretPosition: number, fieldOptions: Record<string, string[]>) {
            ...
            const filtered = word ? allFields.filter((f) => f.toLowerCase().startsWith(word.toLowerCase())) : allFields;

 ...
 */
export function getSchemaDefaults<T extends SchemaDefinition>(schema: T): InferSchemaType<T> {
...
export function parseState<T extends SchemaDefinition>(
    ...
    for (const [key, fieldBuilder] of Object.entries(schema)) {
        ...
        const parsed = (fieldBuilder as FieldBuilder<unknown>)._config.parse(strValue);
        if (parsed !== null) {
            result[key] = parsed;
        }
    }
 ...
 */
export function validateState<T extends SchemaDefinition>(schema: T, state: unknown): InferSchemaType<T> {
 ...
 */
export function isStateEqual<T extends Record<string, unknown>>(a: T, b: T): boolean {
 ...
 */
export function stateToSearchString<T extends SchemaDefinition>(schema: T, state: Partial<InferSchemaType<T>>): string {

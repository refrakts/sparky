            ...
            if (config.type === 'array' && config.itemConfig) {
                ...
                return createFieldBuilder({
                    ...
                    serialize: (value: T) => {
                        ...
                        return value.map((item) => itemConfig.serialize(item)).join(separator);
                    },
                    parse: (str: string) => {
...
function stringLiteral<T extends readonly string[]>(literals: T): FieldBuilder<T[number] | null> {
...
function array<T>(itemBuilder: FieldBuilder<T>): FieldBuilder<T[]> {
    ...
    const defaultDelimiter = itemConfig.type === 'number' ? SLIDER_DELIMITER : ARRAY_DELIMITER;
    ...
    return createFieldBuilder<T[]>({
        ...
        serialize: (value) => {
            ...
            return value.map((item) => itemConfig.serialize(item)).join(defaultDelimiter);
        },
        parse: (str) => {
            if (!str) return [];
            const items = str.split(defaultDelimiter);
            const parsed = items.map((item) => itemConfig.parse(item)).filter((item): item is T => item !== null);
            return parsed;
        },
    });
}
